...
const GenerateImages = () => {
    const imageStyle = ['Realistic', 'Ghibli style', 'Amine style', 'Cartoon style', 'Fantasy style', 'Realistic style', '3D style', 'Portrait style']
    ...
    return (
        <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
            ...
            <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg-border border-gray-200'>
                ...
                <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='Describe what to see in the image..' required />
                ...
                <div className='my-6 flex items-center gap-2'>
                    <label className="relative cursor-pointer">
                        ...
                        <span className='absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition peer-chacked:translate-x-4'></span>
                    </label>
                ...
                <button disabled={loading} className='w-full flex justify-center item-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
                   {
            ...
            <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
...

                {
                    ...
                    <div className='text-sm flex flex-col items-center gap-5 taxt-gray-400'>
                        <Image className='w-9 h-9' />
                        <p>Enter a tropic and click "Generate Image" to get started.</p>
                    </div>
                </div>
                    ) : (
...
//             // API returns { success: true, content: "<url>" }
//             if (data.success) {
//                 setContent(data.content)
//             } else {
...
//                         <input type="checkbox" onChange={(e) => setPublish(e.target.checked)} checked={publish} className='sr-only peer' />
//                         <div className='w-9 h-5 bg-slate-300 rounded-full peer-checked:bg-green-500 transition' />
//                         <span className={`absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition ${publish ? 'translate-x-4' : ''}`} />
//                     </label>
//                     <p className='text-sm'>Make this image Public</p>
//                 </div>
...
//                     {
//                         loading ? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin' /> : <Image className='w-5' />
//                     }
//                     Generate Image
//                 </button>
//             </form>
